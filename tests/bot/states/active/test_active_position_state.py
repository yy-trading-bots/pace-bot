...
class FakePrevState:
    def __init__(self, parent: Any, is_pending: bool) -> None:
        self.parent = parent
        self.is_pending = is_pending
        self._tp_state_obj = object()
        self._sl_state_obj = object()

    def _set_tp_state(self):
        return self._tp_state_obj

    def _set_sl_state(self):
        return self._sl_state_obj
...
def make_instance(
    is_pending: bool, tp: float = 150.0, sl: float = 80.0
) -> ConcreteOpen:
    parent = Parent()
    prev = FakePrevState(parent=parent, is_pending=is_pending)
    return ConcreteOpen(
        parent=parent,
...
def test_init_sets_prices_as_float():
    instance = make_instance(is_pending=False, tp=150.0, sl=80.0)
...
def test_get_position_result(position: PositionSide, is_tp: bool, expected: str):
    instance = make_instance(is_pending=False, tp=100.0, sl=90.0)
    assert instance._get_position_result(position=position, is_tp=is_tp) == expected


def test_handle_tp_when_not_pending(monkeypatch):
    instance = make_instance(is_pending=False, tp=100.0, sl=90.0)
    ...
    assert instance.parent.state is instance.previous_state._set_tp_state()


def test_handle_tp_when_pending(monkeypatch):
    instance = make_instance(is_pending=True, tp=100.0, sl=90.0)
    snapshot = FakeMarketSnapshot(price=200.0)

    success_logs: list[str] = []
    monkeypatch.setattr(
        open_pos_module.Logger, "log_success", lambda msg: success_logs.append(msg)
    )
    monkeypatch.setattr(open_pos_module.FileUtils, "save_result", lambda **kwargs: None)
...
def test_handle_sl_when_not_pending(monkeypatch):
    instance = make_instance(is_pending=False, tp=100.0, sl=90.0)
    ...
    assert saved["result"] == "LONG"
    assert saved["snapshot"] is snapshot
    assert instance.parent.state is instance.previous_state._set_sl_state()


def test_handle_sl_when_pending(monkeypatch):
    instance = make_instance(is_pending=True, tp=100.0, sl=90.0)
    snapshot = FakeMarketSnapshot(price=111.0)
...
def test_close_position_calls_handler_and_logs(monkeypatch):
    instance = make_instance(is_pending=False, tp=100.0, sl=90.0)
    entry_snapshot = FakeMarketSnapshot(price=99.0)
    instance.parent.data_manager.position_snapshot = cast(Any, entry_snapshot)
    ...
    assert calls["tracker"] is instance.parent.performance_tracker
    assert (
        info_logs
...
def test_calling_base_abstract_methods_via_super_raises():
    instance = make_instance(is_pending=False)
    with pytest.raises(NotImplementedError):
        getattr(ActivePositionState, "_is_tp_price")(instance)
    with pytest.raises(NotImplementedError):
        getattr(ActivePositionState, "_is_sl_price")(instance)

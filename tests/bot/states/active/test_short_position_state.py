from typing import Any, cast
from bot.states.active.short_position_state import ShortPositionState
from bot.states.flat.flat_position_state import FlatPositionState


class Snapshot:
    def __init__(self, price: float) -> None:
        self.price = price


class DataManager:
    def __init__(self, snapshot: Snapshot) -> None:
        self.market_snapshot = snapshot


class Parent:
    def __init__(self, price: float) -> None:
        self.data_manager = DataManager(Snapshot(price))


class FakePrevState:
    def __init__(self, parent: Any) -> None:
        self.parent = parent
        self.is_pending = False

    def _set_tp_state(self):
        return object()

    def _set_sl_state(self):
        return object()


def make_sps(price: float, tp: float, sl: float) -> ShortPositionState:
    parent = Parent(price=price)
    prev = FakePrevState(parent)
    return ShortPositionState(
        parent=parent,
        previous_state=cast(FlatPositionState, prev),
        target_prices=[tp, sl],
    )


def test_is_tp_price_true_and_boundary():
    sps = make_sps(price=79.0, tp=80.0, sl=120.0)
    assert sps._is_tp_price() is True
    sps.parent.data_manager.market_snapshot.price = 80.0
    assert sps._is_tp_price() is False


def test_is_sl_price_true_and_boundary():
    sps = make_sps(price=121.0, tp=80.0, sl=120.0)
    assert sps._is_sl_price() is True
    sps.parent.data_manager.market_snapshot.price = 120.0
    assert sps._is_sl_price() is False


def test_apply_triggers_tp_branch(monkeypatch):
    sps = make_sps(price=70.0, tp=80.0, sl=120.0)
    calls = {}

    def fake_close(side, handler):
        calls["side"] = side
        calls["handler"] = handler

    monkeypatch.setattr(sps, "_close_position", fake_close)
    sps.apply()
    assert calls["side"] == "SHORT"
    assert calls["handler"].__name__ == "_handle_tp"
    assert getattr(calls["handler"], "__self__", None) is sps


def test_apply_triggers_sl_branch_when_no_tp(monkeypatch):
    sps = make_sps(price=130.0, tp=80.0, sl=120.0)
    calls = {}

    def fake_close(side, handler):
        calls["side"] = side
        calls["handler"] = handler

    monkeypatch.setattr(sps, "_close_position", fake_close)
    sps.apply()
    assert calls["side"] == "SHORT"
    assert calls["handler"].__name__ == "_handle_sl"
    assert getattr(calls["handler"], "__self__", None) is sps


def test_apply_no_action_when_neither_condition(monkeypatch):
    sps = make_sps(price=100.0, tp=80.0, sl=120.0)
    called = {"n": 0}

    def fake_close(*_args, **_kwargs):
        called["n"] += 1

    monkeypatch.setattr(sps, "_close_position", fake_close)
    sps.apply()
    assert called["n"] == 0


def test_apply_prefers_tp_when_both_true(monkeypatch):
    sps = make_sps(price=100.0, tp=120.0, sl=80.0)
    calls = {}

    def fake_close(side, handler):
        calls["side"] = side
        calls["handler"] = handler

    monkeypatch.setattr(sps, "_close_position", fake_close)
    sps.apply()
    assert calls["side"] == "SHORT"
    assert calls["handler"].__name__ == "_handle_tp"
    assert getattr(calls["handler"], "__self__", None) is sps
